...
export default function ProductCard({ product, viewMode = 'grid' }) {
  ...
  const handleAdd = (e) => {
    ...
    if (needsPrescription && !prescription) {
      // Prescription-required items require customer authentication.
      // We redirect to the main login/sign-up flow (no recreated pharmacy auth UI).
      if (!isLoggedIn) {
        alert('Please log in to upload prescription and order Rx medicines.');
        navigate('/auth/patient/login', { state: { from: `/pharmacy/product/${id}` }, replace: true });
        return;
      }
