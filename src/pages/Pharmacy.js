...
const fallbackCategories = [
  ...
  { id: 6, name: 'Prescription', product_count: 0 },
];
...
export default function Pharmacy() {
  ...
  const filterMockProducts = (nextCategory, nextSearch) => {
    const s = (nextSearch || '').toLowerCase().trim();
    return mockProducts.filter((p) => {
      ...
      const matchCategory = nextCategory === 'All' || p.category === nextCategory;
  ...
  const loadProducts = async () => {
    try {
      ...
      const params = {
        ...
        // Show more medicines by default for pharmacy customers.
        per_page: 40,
      };
      ...
      const nextProducts = Array.isArray(response?.data?.data)
        ...
        : Array.isArray(response?.data)
          ? response.data
          : [];
      // If API returns an unexpected shape (or empty), show the previous mock medicines again.
      const mockFiltered = filterMockProducts(category, search);
      setProducts(nextProducts.length > 0 ? nextProducts : mockFiltered);
    } catch (error) {
      console.error('Failed to load products:', error);
      // Use mock products as fallback
      setProducts(filterMockProducts(category, search));
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadFeaturedProducts = async () => {
    try {
      const response = await productService.getAll({ featured: true, per_page: 12 });
      const nextFeatured = Array.isArray(response?.data?.data)
        ? response.data.data
        : Array.isArray(response?.data)
          ? response.data
          : [];
      setFeaturedProducts(nextFeatured.length > 0 ? nextFeatured : mockProducts.slice(0, 8));
    } catch (error) {
