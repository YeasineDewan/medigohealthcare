...
export default function ProfessionalAdminDashboard() {
  const [selectedPeriod, setSelectedPeriod] = useState(
    () => localStorage.getItem('medigo-admin-selectedPeriod') || 'month'
  );
  const [searchQuery, setSearchQuery] = useState(
    () => localStorage.getItem('medigo-admin-searchQuery') || ''
  );
  ...
  useEffect(() => {
    localStorage.setItem('medigo-admin-selectedPeriod', selectedPeriod);
  }, [selectedPeriod]);

  useEffect(() => {
    localStorage.setItem('medigo-admin-searchQuery', searchQuery);
  }, [searchQuery]);
  ...
  const criticalAlerts = filteredActivity.filter((item) => item.priority === 'critical').length;
  const completionRate = stats.todayAppointments
    ? Math.round(((stats.completedAppointmentsToday || 0) / stats.todayAppointments) * 100)
    : null;

  const exportActivitiesCsv = () => {
    const rows = [
      ['User', 'Action', 'Department', 'Priority', 'Time'],
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden md:flex items-center gap-2 px-3 py-2 bg-white rounded-xl border border-gray-200 shadow-sm">
              <Search className="w-4 h-4 text-gray-400" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search activity or appointments..."
                className="text-sm w-64 focus:outline-none"
              />
            </div>
            <button
              onClick={refetch}
              className="inline-flex items-center gap-2 px-3 py-2 bg-white rounded-xl border border-gray-200 hover:bg-gray-50 shadow-sm"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          <p className="text-xs uppercase tracking-wide text-gray-500">Filtered Activity</p>
          <p className="text-2xl font-bold text-gray-900 mt-1">{filteredActivity.length}</p>
          <p className="text-xs text-gray-500 mt-1">Based on current search query</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          <p className="text-xs uppercase tracking-wide text-gray-500">Appointment Completion</p>
          <p className="text-2xl font-bold text-green-600 mt-1">{completionRate === null ? '--' : `${completionRate}%`}</p>
          <p className="text-xs text-gray-500 mt-1">Completed vs total scheduled today</p>
        </div>
      </div>
