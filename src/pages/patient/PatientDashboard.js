...
const weeklyHealthActivity = {
  exercise: [75, 82, 60, 90, 70, 88, 65],
  sleep: [72, 80, 78, 74, 85, 82, 76],
  hydration: [68, 72, 80, 75, 84, 77, 70],
};

export default function PatientDashboard() {
  const [activeTab, setActiveTab] = useState('overview');
  const [selectedTimeRange, setSelectedTimeRange] = useState(
    () => localStorage.getItem('medigo-patient-selectedTimeRange') || 'week'
  );
  const [notificationFilter, setNotificationFilter] = useState(
    () => localStorage.getItem('medigo-patient-notificationFilter') || 'all'
  );
  ...
  const adherenceScore = Math.round(((weeklyHealthActivity.exercise[6] + weeklyHealthActivity.sleep[6] + weeklyHealthActivity.hydration[6]) / 3));

  useEffect(() => {
    localStorage.setItem('medigo-patient-selectedTimeRange', selectedTimeRange);
  }, [selectedTimeRange]);

  useEffect(() => {
    localStorage.setItem('medigo-patient-notificationFilter', notificationFilter);
  }, [notificationFilter]);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Link to="/patient/book">
          <motion.div
            ...
            className="p-4 rounded-xl bg-gradient-to-br from-[#5DBB63] to-[#4a9a4f] text-white hover:shadow-lg transition-all"
          >
            <Calendar className="w-6 h-6 mb-2" />
            <span className="text-sm font-medium">Book Doctor</span>
          </motion.div>
        </Link>
        <Link to="/patient/live-consult">
          <motion.div
            whileHover={{ scale: 1.02 }}
            className="p-4 rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 text-white hover:shadow-lg transition-all"
          >
        ...
        <Link to="/patient/orders">
          <motion.div
            whileHover={{ scale: 1.02 }}
            className="p-4 rounded-xl bg-gradient-to-br from-amber-500 to-amber-600 text-white hover:shadow-lg transition-all"
          >
        ...
        <Link to="/patient/records">
          <motion.div
            whileHover={{ scale: 1.02 }}
            className="p-4 rounded-xl bg-gradient-to-br from-purple-500 to-purple-600 text-white hover:shadow-lg transition-all"
          >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          <div className="rounded-xl bg-[#5DBB63]/10 p-4">
            <p className="text-xs text-gray-600">Wellness Adherence</p>
            <p className="text-2xl font-bold text-[#165028]">{adherenceScore}%</p>
            <p className="text-xs text-gray-500">Based on exercise, sleep, hydration</p>
          </div>
          <div className="rounded-xl bg-blue-50 p-4">
            <p className="text-xs text-gray-600">Weekly Goal Completion</p>
            <p className="text-2xl font-bold text-blue-700">5 / 7</p>
            <p className="text-xs text-gray-500">Healthy routine days completed</p>
          </div>
          <div className="rounded-xl bg-amber-50 p-4">
            <p className="text-xs text-gray-600">Risk Signal</p>
            <p className="text-2xl font-bold text-amber-700">Low</p>
            <p className="text-xs text-gray-500">No critical trend detected this week</p>
          </div>
        </div>
                ...
                <div key={day} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-gray-200 rounded-full h-2">
                    <div 
                      ...
                      style={{ width: `${weeklyHealthActivity.exercise[i]}%` }}
                    />
                  </div>
                </div>
              ))}
                ...
                <div key={day} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-gray-200 rounded-full h-2">
                    <div 
                      ...
                      style={{ width: `${weeklyHealthActivity.sleep[i]}%` }}
                    />
                  </div>
                </div>
              ))}
                ...
                <div key={day} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-gray-200 rounded-full h-2">
                    <div 
                      ...
                      style={{ width: `${weeklyHealthActivity.hydration[i]}%` }}
                    />
                  </div>
                </div>
              ))}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
          <h2 className="font-semibold text-[#111827] text-lg">Notification Center</h2>
          <select
        ...
        <div className="space-y-2">
          {filteredNotifications.map((notif) => (
            <div key={notif.id} className={`p-3 rounded-lg border ${notif.read ? 'border-gray-200 bg-gray-50' : 'border-[#5DBB63]/30 bg-[#5DBB63]/10'}`}>
