...
export default function Login({ role = 'patient' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      ...
      const mockUser = {
        id: Date.now(),
        name: formData.email?.split('@')[0] || `${roleLabel} User`,
  ...
  const handleBiometricLogin = async ({ imageBlob, fingerprintTemplate }) => {
    // In the current project, auth is still mocked (no backend biometric endpoint).
    // We keep the captured biometric data for auditing/demo and complete the same login flow.
    setIsSubmitting(true);
    try {
      // Persist only locally for demo; do not store sensitive templates in real production.
      try {
        if (imageBlob) {
          localStorage.setItem('biometric_image_captured_at', new Date().toISOString());
        }
        if (fingerprintTemplate) {
          localStorage.setItem('biometric_fingerprint_captured_at', new Date().toISOString());
        }
      } catch (e) {
