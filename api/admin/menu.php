<?php
/**
 * Admin Menu API Endpoint
 * Returns menu structure based on user permissions
 */

require_once '../config.php';

// Get current user (optional - if not authenticated, return full menu)
...
$menuStructure = [
    ...
    [
        ...
        'icon' => 'FileText',
        'order' => 5,
        'roles' => ['admin', 'super_admin', 'doctor', 'lab_technician'],
...
function filterMenuByPermissions($menuItems, $userRole = null, $permissions = []) {
    return array_filter($menuItems, function($item) use ($userRole, $permissions) {
        // Check role-based access
        if (isset($item['roles']) && !empty($item['roles'])) {
            if (!$userRole || (!in_array($userRole, $item['roles']) && !in_array('super_admin', $item['roles']))) {
                return false;
            }
        }
        ...
        if (isset($item['children']) && is_array($item['children'])) {
            $item['children'] = filterMenuByPermissions($item['children'], $userRole, $permissions);
...
// Get user permissions from database if authenticated
$userRole = null;
$userPermissions = [];

if ($currentUser) {
    $userData = $db->fetchOne(
        "SELECT r.name as role, r.permissions as role_permissions 
         FROM users u 
    ...
    if ($userData) {
        ...
        $rolePermissions = json_decode($userData['role_permissions'] ?? '{}', true);
        $userPermissions = array_keys($rolePermissions);
    }
}

// Filter menu based on permissions
$filteredMenu = filterMenuByPermissions($menuStructure, $userRole, $userPermissions);

// Return filtered menu
apiResponse(array_values($filteredMenu));
?>
